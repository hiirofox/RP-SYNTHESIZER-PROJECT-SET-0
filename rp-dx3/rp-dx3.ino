...
public:
  ...
  void UpdataADC() {
    ...
    Vp1 = adcfilt1.LPF1(tmpv1, 0.3, 0) / 4095.0;
    Vp2 = adcfilt2.LPF1(tmpv2, 0.3, 0) / 4095.0;
    Vp3 = adcfilt3.LPF1(tmpv3, 0.3, 0) / 4095.0;
    Vp1 = Vp1 * Vp1;
    Vp2 = Vp2 * Vp2;
    Vp3 = Vp3 * Vp3;
    ...
    Vd1 = adcfilt7.LPF1(tmpv7, 0.3, 0) / 4095.0 * 1.1;
    Vd2 = adcfilt8.LPF1(tmpv8, 0.3, 0) / 4095.0 * 1.1;
    Vd3 = adcfilt9.LPF1(tmpv9, 0.3, 0) / 4095.0 * 1.1;
    Vd1 = 1.0 - (1.0 - pow(Vd1, 1.0 / 4.0)) * 0.005;
    Vd2 = 1.0 - (1.0 - pow(Vd2, 1.0 / 4.0)) * 0.005;
    Vd3 = 1.0 - (1.0 - pow(Vd3, 1.0 / 4.0)) * 0.005;
    Vd1 = Vd1 > 0.999998 ? 0.999998 : Vd1;
    Vd2 = Vd2 > 0.999998 ? 0.999998 : Vd2;
    Vd3 = Vd3 > 0.999998 ? 0.999998 : Vd3;
    Vf2 = (int)(adcfilt10.LPF1(tmpv10, 0.3, 0) / 4095.0 * 16.0) + 1.0;
    Vf3 = (int)(adcfilt11.LPF1(tmpv11, 0.3, 0) / 4095.0 * 16.0) + 1.0;
    Vfdbk = adcfilt12.LPF1(tmpv12, 0.3, 0) / 4095.0 * 4096.0;
  }
  ...
  void AutoTrig(int freq) {  //自动分配振荡器
    ops[inPos].SetTrig(freq * 32768 / SAMPLE_RATE);
    inPos = (inPos + 1) % MaxOPN;
  }
  void proc() {
    if (Trig == 1) {
      Trig = 0;
    ...
    for (int i = 0; i < BufLen; i += 2) {
      float dat = 0.0;
      for (int j = 0; j < MaxOPN; ++j) {
        dat += ops[j].proc(Vf2, Vf3, 1.0, Vp2, Vp3, Vfdbk);
        ops[j].UpdataVolume(Vd1, Vd2, Vd3);
      }
      int tmpL = dat * 5000.0;
      int tmpR = dat * 5000.0;
      WavBuf[i + 0] = tmpL;
...
void loop() {
  ...
  for (;;) {
    ...
    RPDX3Proc.UpdataADC();   //更新adc
    RPDX3Proc.UpdataKB();    //更新键盘
    RPDX3Proc.UpdataAlgm();  //更新算法
  }
  wo.Stop();
}
