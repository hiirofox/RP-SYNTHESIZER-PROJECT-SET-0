...
public:
  ...
  void proc() {
    ...
    for (int i = 0; i < BufLen; i += 2) {
      ...
      float datL = 0, datR = 0;
      StereoSignalFloat Sign;
      for (int j = 0; j < MaxStrN; ++j) {
        Sign = strs[j].proc_stereo(Vdecay, Vlpf);
        datL += Sign.l;
        datR += Sign.r;
      }
      datL = InDlyL.LPF1(datL, 0.2, 0);
      datR = InDlyR.LPF1(datR, 0.2, 0);
      datL = ldly.proc(datL, Vldt, Vdfdbk, Vdmix);
      datR = rdly.proc(datR, Vrdt+10, Vdfdbk, Vdmix);
