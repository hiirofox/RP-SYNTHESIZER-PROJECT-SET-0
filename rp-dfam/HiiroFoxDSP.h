...
*/
  ...
  StereoSignal Sqr2_Stereo_Realtime(float SawFreq, int UniN, float delta) {
    //t1 += (SawFreq << 15) / SAMPLE_RATE;
    int tmp2_l = 0, tmp2_r = 0, tmp_switch = 1;
    freq1 = SawFreq + freq1 - (int)freq1, freq2 = SawFreq + freq2 - (int)freq2;
    for (int i = 0; i < UniN; ++i) {
      tmp2_l += ((ts1[i] += (uint32_t)(freq1 += delta)) - 1357) % SAMPLE_RATE > 26000 ? 26000 : -26000;
      tmp2_r += ((ts2[i] += (uint32_t)(freq2 -= delta)) + 2048) % SAMPLE_RATE > 26000 ? 26000 : -26000;
    }
    return (StereoSignal){ (tmp2_l + tmp2_r) / UniN, (tmp2_l - tmp2_r) / UniN};
  }
};
